...
if(isset($_GET['id'])) {
        ...
        echo '<title>Actualizaci&oacuten de fotograf&iacutea</title>';
	...
	include_once("../java.html");
		...
		if (isset($_POST['envia_img'])) {
			...
			//Se verifica si las caracteristicas del archivo son las correctas 
			if (($tipo_archivo[2] == 1) || ($tipo_archivo[2] == 2) || ($tipo_archivo[2] == 3)) {
				if ($tamano_archivo < $max_tamano) { 
			   		if (move_uploaded_file($archivo_tmp, "$dir_imagenes/$nombre_imagen$extension")){ 
						...
						//***Borrar im&aacutegenes (si existen) de diferente tipo.
                        if ($tipo_archivo[2]==1) {
						...
						include_once("encabezado.html");
	    		 		echo "<strong>Ocurri&oacute alg&uacuten error al subir la imagen. No pudo guardarse.</strong><br />(Verifique que el directorio <u>fotos</u> tiene habilitado los permisos de lectura, escritura y ejecuci&oacuten en su sistema)."; 
						echo '<br /><br /><input type=button value="Cerrar" onclick="CerrarVentana()"></div>';
		   			} 
				}
				else {
					include_once("encabezado.html");
					echo "<strong>La imagen que desea subir sobrepasa el tama&ntildeo m&aacuteximo de 4MB.";
        				echo "<br /><a href='javascript:history.go(-1)'>Volver a subir una nueva imagen</a></strong></div>";
				}
			} 
			else {
				echo '<div class="cen"><h2 class="txtinicial">Solamente puede subir im&aacutegenes de tipo jpg, gif o png.<br /> (recuerde que el tama&ntildeo m&aacuteximo de la imagen es de 4MB)</h2>';
			...
			echo '</form>';
			echo '</td></tr></table>';
			echo '<br /><h2 class="txtinicial cen">(El tama&ntildeo m&aacute;ximo para la imagen son 4MB)</h2>';
		}
