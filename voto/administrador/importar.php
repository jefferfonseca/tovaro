...
if (!isset($_POST['envia_csv'])) {
    ...
    echo '<ol>';
    echo '<li>Grado del estudiante (num&uacute): Debe coincidir con el n&uacutemero del grado que aparece en el sistema.</li>';
    echo'<li>Nombres.</li>';
    echo '<li>Apellidos.</li>';
    echo '<li>Documento (sin puntos ni comas).</li>';
    echo '<li>contrase&ntildea (opcional).</li>';
    echo '</ol>';
	echo '<strong><u>Tenga en cuenta que al importar el archivo CSV, se borrar&oacuten los datos de votaci&oacuten y los estudiantes existentes.</u></strong></td></tr></table><br />';
	echo '<table><tr><td>';
	...
	echo '</form>';
	echo '</td></tr></table>';
	echo '<p style="font-size:10px; text-align:center;">(El tama&ntildeo m&aacuteximo del archivo CSV es de 10MB)</p>';
    echo '</div></body></html>';
}
else {
	...
	if (isset($_POST['envia_csv'])) {
		...
		//Se verifica si las caracter&iacutesticas del archivo son las correctas 
		if (($tipo_archivo == "application/vnd.ms-excel")||($tipo_archivo == "text/csv")||($tipo_archivo=="text/comma-separated-values")) {
			if ($tamano_archivo < $max_tamano) { 
		   		if (move_uploaded_file($archivo_tmp, "$dir_csv/$nombre_csv")){ 
					...
					if (($gestor = fopen("csv/estudiantes.csv", "r")) !== FALSE) {
						...
						if((count($data2)>3)and(count($data2)<6)) {
							...
							echo "N&uacutemero de registros leidos...".$fila."<br />";
							echo "Tama&ntildeo del archivo...".$tamano_archivo." bytes<br /><br /></div>";
						}
						else {
								include_once("encabezado.html");
								echo "<h2>Error</h2>";
    		 					echo "<strong>El archivo CSV debe contener m&acutenimo cuatro (4) y m&aacuteximo cinco (5) campos por registro<br />(grado, nombres, apellidos, documento y clave -opcional-).<br /> Tambi&eacuten debe tener en cuenta que el caracter separador de campos en el archivo CSV, sea punto y coma(;).<br /> Por favor verif&iacuteque de nuevo su archivo CSV.</strong><br /><br /></div>"; 
						}
					}
					include_once("../java.html");
					...
					include_once("encabezado.html");
						echo "<h2>Error</h2>";
    		 			echo "<strong>Ocurri&oacute alg&uacuten error al intentar subir el archivo.  Verifique la estructura de su archivo CSV o si tiene permisos de lectura, escritura y ejecuci&oacuten para el directorio csv.</strong>"; 
				...
				include_once("encabezado.html");
				echo "<h2>Error</h2>";
				echo "<strong>El archivo que desea subir sobrepasa el tama&ntildeo m&aacuteximo de 10MB.";
       				echo "<br /><a href='javascript:history.go(-1)'>Volver a subir un nuevo archivo</a></strong></div>";
			}
		} 
		else {
