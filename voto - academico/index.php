...
if ($_POST['documento']!="") {
	...
	print "<strong>No ha escrito el n&uacutemero de documento<br />";
	print"<br /><a href='javascript:history.go(-1)'>Volver al formulario</a></strong></div></body></html>";
	exit;
}
//****Se valida la contrase&ntildea del estudiante si el sistema la solicita
if ($leer['clave']=="S") {
	if ($_POST['contra']!="") {
	...
	if (!$row6= mysql_fetch_array($resp6)) {
		include_once("encabezado.html");
		print "<strong>La contrase&ntildea de acceso es inv&aacutelida<br />";
		print"<br /><a href='javascript:history.go(-1)'>Volver al formulario</a></strong></div></body></html>";
		exit;
	}
}
...
$resp2=mysql_query(sprintf("select id_estudiante from voto, estudiantes where documento=%s and id_estudiante=estudiantes.id",comillas($DocEst)),$link);
		...
		include_once("encabezado.html");
		print "<strong>No puede ingresar</strong><br />Su voto ya est&aacute registrado en el sistema.<br />";
	...
	if ($row= mysql_fetch_array($resp)) {
		...
		echo '<tr>';
		echo '<td>';
        //Leemos la lista de categorias que apareceran en el tarjeton
		$resp5=mysql_query("select * from categorias order by id",$link);
		while($row5 = mysql_fetch_array($resp5)) {
            //Verificamos si existe un grado con el mismo nombre de la categoria
        ...
        //Se muestran los candidatos por grado (pertenecen al mismo grado del estudiante) o de otras categorias
		...
		//***Si el tarjetn no tiene candidatos se muestra un mensaje
        if ($catarj=="") {
            echo '<strong>No existen candidatos para votar, por favor comun&iacutequese con el administrador del sistema.</strong>';
            echo '</td>';		
    		echo '</tr>';
	    	echo '</table>';
		    echo '</div><br />';
    		...
    		//Eliminamos la ultima coma "," de la lista de categorias
            $catarj=trim($catarj,',');
		...
		echo '<table>';
		echo '<tr><td class="cen" colspan="2"><strong>El documento escrito no est&aacute registrado en el sistema<br /><br />';
    ...
    include_once("encabezado.html");
	print "<strong>EL SISTEMA DE VOTACI&OacuteN ESTA INACTIVO</strong><br />";
    print "(Comun&iacutequese con el administrador del sistema)</div></body></html>";
}
mysql_close($link);
?>
